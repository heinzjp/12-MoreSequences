...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  55 slices, thin (thickness=5) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    pizza(window, circle, 55, 'red', 5)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # Tests 3:
    title = 'POLYGON tests 3:  8 segments with red lines'
    window = rg.RoseWindow(550, 400, title)
...
def run_test_fancy_polygon():
    ...
    # Test 3 (on another window):
    title = 'FANCY POLYGON test 3:  300 lime green lines on blue circle'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        point = points[k]
        point2 = points[(k + hops_to_next_point) % len(points)]
        line = rg.Line(point, point2)
        line.attach_to(window)
        line.color = color
        line.thickness = thickness
